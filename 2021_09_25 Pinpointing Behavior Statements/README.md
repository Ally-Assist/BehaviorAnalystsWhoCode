...
A very popular form of self defense is commonly known as [design by contract](https://en.wikipedia.org/wiki/Design_by_contract). Minimally, it includes checks at the beginning of each method to ensure that the object is in a valid state, and that the arguments passed in are valid. These conditions are generally documented in comments for the class and for each method.
...
Having mentioned commenting, I would be remiss in not mentioning a very popular tool for generating documentation from comments in code, so long as the commenting conventions of the programming language are followed. If you use a lot of modules written by other people, in almost any language, you will have seen documentation generated by this tool:

- [Doxygen](https://www.doxygen.nl/index.html)

The requirements for use with Python are minimal:

- [Comment blocks in Python](https://www.doxygen.nl/manual/docblocks.html#pythonblocks)

Simple example documentation:

- [Documenting Python Programs With Doxygen](https://www.woolseyworkshop.com/2020/06/25/documenting-python-programs-with-doxygen/#ViewingTheGeneratedDocumentation)
